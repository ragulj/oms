...
| **Idempotent Creation** | Spec 003 | No replay protection on order creation; treat a client retry as the client's problem. | An optional `Idempotency-Key` header, recorded in the same transaction that creates the order, with a replay answering 200. | Principle IV makes a stored order permanent: a duplicate created by a network retry can never be deleted, only cancelled. Refusing it at the door is the only remedy this system actually has. The guarantee is the unique constraint, not the lookup that precedes it, so a request that loses the race still fails on the constraint. |
| **Cancellable States** | Spec 003 | Allow cancelling an order that is already `processing`. | Only a `pending` order can be cancelled. | The status set carries no completion or compensation value, so cancelling after promotion would leave the order in a state the model cannot describe. It also gives the state machine a business meaningful edge to refuse, rather than only self transitions. |
| **Keyset Predicate** | Spec 003 | `created_at_us < ? OR (created_at_us = ? AND id < ?)`, which Spec 002's own test used. | A row value comparison, `(created_at_us, id) < (?, ?)`. | Measured, not assumed. Under a status filter the OR form constrains only `status` and then walks every row of it, which is a full scan wearing an index's name. The row value form constrains the timestamp too and seeks straight to the cursor. Both query plans are recorded in Spec 003 research R4. |
| **Error Envelope Scope** | Spec 003 | Register the exception filter globally, as the framework docs suggest. | Bind it to the orders controller only. | The global filter silently rewrote the health endpoint's response body and broke a Spec 001 test. The envelope is the order API's contract, not the whole process's, and one feature should not redefine another's contract as a side effect. |
| **Transport Types** | Spec 003 | Install `@types/express` to type the middleware, filter, and controller. | Declare the handful of properties the HTTP layer actually touches. | Keeps the feature at zero new dependencies, and puts the surface the HTTP layer depends on in one readable file instead of an open ended framework type. |
