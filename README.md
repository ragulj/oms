...
| **Scheduler loop termination** | Spec 005 | Keep Spec 003's rule that only a zero-row claim ends a tick. | End the tick on any claim shorter than the chunk size. | Spec 003 chose the zero-row rule deliberately and wrote down why: the claim's outer status predicate might exclude an order cancelled "between the subquery choosing it and the update reaching it", so a short chunk would not prove the backlog was empty. Measured against a real database, that interval does not exist — the claim is one statement in one transaction on an engine that serialises writers, so nothing can interleave within it. The reasoning was sound; the premise was wrong. The confirming claim, one wasted write transaction per tick against a short or empty backlog, is gone. |
| **Where the iteration cap lives** | Spec 005 | Move the cap out of the loop condition into an internal guard, which is what "the cap must stop being the normal stopping condition" literally asks for. | Leave the cap as the `while` condition and add the short-claim exit inside it. | Both readings satisfy the requirement behaviourally, and only one of them keeps an unbounded loop unrepresentable. The driver is synchronous, so a claim loop that lost its bound would hold the single write lock and block the event loop for as long as it ran — Constitution Principle III calls the cap a safety property, not a tuning knob. Structure that cannot express the unsafe state beats structure that matches the sentence. The mutation sweep keeps a `while (true)` variant precisely because it is the tempting misreading. |
