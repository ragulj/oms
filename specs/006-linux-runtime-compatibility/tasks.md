...
- [ ] T031 ⚠️ **DOES NOT REPRODUCE** (attempted 2026-09-05 on Windows 11, Node v24.19.0, npm 11.17.0, at `db8ee43`). Three consecutive `npm test` runs: **65/65 suites, 468/468 tests, exit 0**, no `globalTeardown` failure, and `%TEMP%\oms-test\` empty afterwards — `removeTestDatabase` removed all three files. There is no failure here to baseline, so FR-025's required evidence cannot be captured as written and T032 has no retained resource to identify. **Path correction**: the task text below said `data/oms-test.db`; the suite's database is `join(tmpdir(), 'oms-test', 'oms-test.db')` per `test/setup/database.ts:11-12`. `data/` holds only the development database `oms.db*`. Leading (unconfirmed) hypothesis for the disappearance: `51df411` changed `test/support/http-fixtures.ts` so the harness binds its server once for its lifetime instead of supertest binding lazily per request and closing it again — per-request bind-and-close is the shape that strands handles on Windows. Confirming that needs a bisect to `93735c6`, which was offered and declined. **Disposition owed** — see the Phase 8 note below. Capture the failure baseline on the previously supported environment: run `npm test`, confirm `globalTeardown` fails, and record the exact error plus which of the three files `%TEMP%\oms-test\oms-test.db`, `…-wal`, `…-shm` cannot be removed by `removeTestDatabase` in `test/setup/database.ts`. This is FR-025's required evidence and the baseline the fix is judged against (FR-025, SC-006)
- [ ] T032 ⚠️ **NOT APPLICABLE AS WRITTEN** (2026-09-05) — depends on a failure T031 could not reproduce. There is nothing to bisect and no holder to identify: the suite is green on Windows and teardown removes all three files. Re-open only if the failure recurs, or if the `93735c6` bisect is run and confirms `51df411` removed it. Identify the specific retained resource. Bisect with `npm test -- --testPathPatterns <suite>` to the smallest set that reproduces the failure, and confirm the holder against the Jest process. Do not assume `test/integration/docs/response-conformance.spec.ts` is the culprit — its connection is already closed at line 305, so the retained handle is something else. Record the finding in [research.md](research.md) (FR-025)

### Cleanup hygiene (independent of diagnosis; not acceptance on its own)

- [X] T033 [P] ✅ DONE (2026-09-05, Windows) — `response-conformance.spec.ts:312` now calls `await degraded.close()`; line 305 untouched. Confirmed `Connection.close` guards on `sqlite.open` at `src/database/client.ts:61-63`, with a comment already anticipating this exact double-close. Suite green after the change (65/65, 468/468). Route the degraded application's teardown through the harness routine in `test/integration/docs/response-conformance.spec.ts`: replace `await degraded.app.close()` in the `finally` at line 312 with `await degraded.close()`. **Keep line 305 (`degraded.connection.close()`) exactly as it is** — it is the arrangement that provokes the 503, not cleanup, and removing it breaks the test. `close()` is idempotent (`Connection.close` guards on `sqlite.open`), so the second close is safe (FR-024)
- [X] T034 [P] ✅ DONE (2026-09-05) — three-line comment added above the `drain({...})` call stating that the teardown mechanism is the subject under test, that routing through `harness.close()` would hide the asserted behaviour, and that the connection is still released below. The bypass is now explained rather than merely present. Record the FR-024 exemption at the deliberate bypass in `test/integration/shutdown.drain.spec.ts` lines 33-40: it passes `harness.app.close` into `drain()` because the teardown mechanism is the subject under test, and releases `harness.connection` separately at line 39. Add a comment stating why it bypasses `harness.close()`, satisfying the conditional carve-out — the exemption must be explained, never merely present (FR-024)
- [X] T035 [P] ✅ DONE (2026-09-05) — re-ran the grep after T033. Exactly one `.app.close()` call site remains, `test/integration/shutdown.drain.spec.ts:40`, which is the documented exemption from T034. No third bypass. Confirm no third bypass exists. The 2026-09-05 audit found exactly two across 13 `createTestApp` call sites, the other eleven routing through `harness.close()`. Re-run `grep -rn '\.app\.close()' test/ --include=*.ts` after T033 and confirm only the documented `shutdown.drain.spec.ts` exemption remains (FR-024)
- [X] T036 [P] ✅ DONE (2026-09-05) — `removeTestDatabase` still loops the three suffixes and issues one `rmSync(path, { force: true })` each (`test/setup/database.ts:15-22`). Zero occurrences of `maxRetries`, `retryDelay`, or `backoff` in the file. No retry was added, so FR-026 holds. Confirm `removeTestDatabase` in `test/setup/database.ts` still performs a single `rmSync(path, { force: true })` per file, with no `maxRetries`, `retryDelay`, backoff, or retry loop added. FR-026 forbids making the suite pass by retrying deletion; note `force` only suppresses ENOENT and must not be mistaken for handling a locked file (FR-026)

### Acceptance

- [ ] T037 ⚠️ **SATISFIED IN LETTER, NOT IN INTENT** (2026-09-05, Windows) — the stated condition holds: `npm test` is green (65/65, 468/468, exit 0) after T033/T034, and teardown removes all three database files. But FR-025 requires acceptance to rest on the resource identified in T032 having been released, and T032 identified none because nothing failed. Marking this `[X]` would assert a diagnosis that was never made — exactly what FR-025 forbids. Held open pending the disposition decision below. Verify the acceptance condition: a complete `npm test` green on Windows, with teardown removing all three database files. Per FR-025 the presence of the code change is explicitly **not** acceptance — the suite must pass because nothing holds the file, and the resource identified in T032 must be the one released (FR-025, SC-006)
- [ ] T038 ⚠️ **HALF DONE — Windows verified, Linux pending** (2026-09-05). Windows after T033/T034: `npm test` = 65/65 suites, 468/468 tests, exit 0, no existing expectation modified; the diff is 4 lines across two files. The Linux half needs that machine. **Separately discovered and NOT caused by T033/T034**: `npm run check` does **not** exit 0 on this Windows checkout. `prettier --check .` fails on 45 CRLF working-tree files (`git ls-files --eol`: index is `lf` throughout, worktree is 45 `crlf` / 177 `lf` / 1 `mixed`; `core.autocrlf=true`, no `.gitattributes`), so eslint, `tsc --noEmit` and `openapi:check` never run. Both files edited here pass prettier. This falsifies T016's `[X]`, which recorded `npm run check` exit 0 on Linux only and dismissed the CRLF concern as non-reproducing there. See the Phase 8 note below. Verify no Linux regression after T033 and T034: `npm test` returns 468 passed / 65 suites and `npm run check` exits 0 on Linux under Node 24.20.0, with no existing test's expectation modified (FR-016, SC-006)
- [ ] T039 ⏸️ **HELD** (2026-09-05) — depends on T037, which is held pending the disposition decision. Rewriting SC-006 from "partially met" to met would assert a close that FR-025's evidence standard has not earned, so the spec text is deliberately left alone rather than updated to match a convenient outcome. (depends on T037) Update the record: change SC-006's status note in [spec.md](spec.md) from "partially met" to met, naming the environment and date; and add the diagnosis to `test/integration/README.md` beside the existing Spec 006 entries, so the next reader inherits the real mechanism rather than the reported one (SC-006)
...
### Disposition owed (added 2026-09-05, after the Windows run)

Two questions this phase cannot answer for itself. Both are spec decisions, and both follow the
precedent of T025, which routed the FR-003 retraction through `/speckit-clarify` rather than quietly
dropping a falsified claim.

1. **How does FR-025 close when its failure will not reproduce?** FR-025 forbids accepting a remedy on
   the code change alone and requires the retained resource to be named on the failing environment.
   There is no failing environment any more. Either the `93735c6` bisect is run so the defect and its
   removal are on record, or FR-024–FR-026 are amended the way FR-003 was — stating plainly that the
   reported failure did not reproduce and that the hygiene changes stand on their own merit. What is
   not acceptable is marking T037 `[X]` and letting SC-006 read as diagnosed when nothing was.

2. **Does the Windows `npm run check` failure belong to this specification?** `prettier --check .` fails
   on a stock Windows checkout because `core.autocrlf=true` and there is no `.gitattributes`, so the
   first of the four gates fails and the other three never run. This is squarely a cross-platform
   defect (FR-011's spirit, FR-016 and FR-017's letter, and SC-006's promise about the previously
   supported environment), and T016's `[X]` rests on a Linux-only run that explicitly waved the concern
   away. But the fix — a `.gitattributes` carrying `* text=auto eol=lf` — is a repository-wide
   normalization, and this phase's scope constraints say test cleanup only. Decide whether it joins
   Spec 006 as the second unfinished half of SC-006, or becomes Spec 007.

**Checkpoint**: SC-006 is met on both environments, and the mechanism on record is the one that was
observed rather than the one that was reported. **Not reached** — see the disposition above.
