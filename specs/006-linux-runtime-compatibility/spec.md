...
- Q: How is Linux startup verified, given the repo has no CI? → A: Via the project's own checks, with no CI pipeline added; the source-loader fix is proven by the existing real-database integration suite loading and running on Linux. (Refined by the fourth clarification below, which adds an observed `start:dev` boot alongside the suite.)
- Q: On Linux, does closing User Story 1 require an actual `start:dev` boot to serving state, or is the automated suite enough? → A: Both are required. The suite is the automated evidence; a manual `npm run start:dev` boot reaching the serving state is also required evidence, recorded in `test/integration/README.md` as a check that cannot be automated from inside the suite (research R4 rejected automating it).
- Q: Do the cross-platform path guarantees need their own automated test, or does the suite passing on Linux confirm them? → A: The suite passing on Linux's case-sensitive filesystem is the confirmation; no dedicated case-sensitivity test is added. FR-011 and FR-012 stay backed by review of the changed code, and FR-023's "turn it red" standard is scoped to the two behavioural guarantees (directory creation and the loader configuration).
- Q: A Windows-only test-cleanup defect (`EPERM` deleting the shared test database) was reported after this feature closed. Does it belong to this specification or a new one? → A: To this specification, as the unfinished half of SC-006. SC-006 has always promised the suite passes on the previously supported environment as well as Linux, and only Linux was ever verified, so this closes an existing obligation rather than adding scope. Tracked as a new phase in the task list, on the same footing as the supertest harness fix recorded under FR-016.
- Q: Should the Windows fix implement the reported cleanup change as stated, or must the actual retained file handle be identified first? → A: Both, in that order of authority. Routing the harness's teardown through its own `close()` is worth doing as hygiene, but it is not accepted as the fix on its own: the retained handle must be identified on the failing environment, and acceptance is a green suite there rather than the presence of the code change. The reported mechanism does not match the code — the connection is already closed before the `finally` — so a fix written to it alone risks changing nothing.
- Q: Does the harness-teardown rule also bind a test whose subject is the shutdown mechanism itself? → A: No — such a test is exempt, on two conditions: it must still release every resource it opened, and it must say in a comment why it bypasses the harness routine. A test that drives the teardown path cannot route through the wrapper that hides it, but the exemption is earned rather than assumed, so review can still enforce the rule.
- Q: May the test-database removal retry on the failing environment, or must a green suite come only from releasing the handle? → A: Only from releasing the handle. No retry, backoff, or force-delete is added to the removal step. A retry would very likely turn the suite green without anyone identifying the retained resource, which would empty FR-025's acceptance test of meaning; in a single-worker, single-process suite a lingering handle is a defect rather than environmental flakiness to absorb.
...
- **A test opens a second application against the shared test database and closes it by hand.** The teardown releases part of what it opened. On a filesystem that permits deleting an open file this is invisible; on one that does not, the shared database cannot be removed and teardown fails. The release path is therefore the harness's own teardown routine, not an ad hoc subset, so the two filesystems behave alike.
...
- **FR-003**: The source loader MUST use a module system and module-resolution strategy that are correct for running the project's sources under the current Node runtime, independent of the module settings chosen for the compiled build. **Amended after implementation (2026-09-05).** The Linux failure this requirement was written to remove turned out to be a corrupt dependency installation, not a loader misconfiguration: the installed package was missing 336 of its 444 type-declaration files, which its own `exports` map declared. With the installation intact, the loader resolves the project's modules correctly under the pre-existing settings on every runtime tested. The loader-scoped configuration is therefore **retained deliberately as hardening**, not as the fix for a reproducible failure — several dependencies publish their entry points only through an `exports` map, and the loader-scoped setting is the resolution mode that honours it. Retention is the decision on record; this requirement no longer asserts that removing it breaks startup.
...
- **FR-021**: Startup under the source loader MUST be verified on Linux by two pieces of evidence, together referred to as **the Linux startup verification**: (a) the project's existing real-database integration suite loading and running to completion on Linux, which is the automated evidence that direct source loading works there; and (b) an observed run of the documented development start command on Linux reaching the serving state, together with each remaining helper script that executes sources directly (migration, seed, and documentation export) running to completion. Part (b) MUST be recorded in the integration suite's README among the checks that cannot be automated from inside the suite, stating the command and the expected outcome, because automating a spawn-and-wait boot would exercise the Node runtime's process handling rather than this service. This specification introduces no continuous-integration pipeline; if one is added later it MUST run part (a) on Linux, but no such pipeline may be a precondition for considering the source-loader fix verified.
...
- **FR-024**: A test that provisions its own application harness MUST release it through that harness's own teardown routine rather than closing a subset of its resources by hand. A teardown that closes some of what the harness opened, while the harness exposes a routine that closes all of it, MUST be treated as a defect even where it currently leaves no observable leak, because the omission is invisible until a platform enforces it. **One exemption**: a test whose subject is the teardown mechanism itself MAY drive that mechanism directly, since routing through the wrapper would hide the behaviour under test. The exemption is conditional — such a test MUST still release every resource it opened, and MUST record at the call site why it bypasses the harness routine — so that a bypass is always either compliant or explained, and never merely overlooked.
- **FR-025**: The failure that motivated FR-024 MUST NOT be treated as fixed on the strength of the code change alone. The specific retained resource MUST be identified on the environment where the failure occurs, and acceptance MUST be the complete suite passing on that environment. A remedy derived from a reported mechanism that the code does not exhibit MUST be re-derived from evidence before it is accepted, and where the reported mechanism and the code disagree, the code and a reproduction are authoritative.
- **FR-026**: The step that removes the throwaway test database MUST NOT be given retry, backoff, or force-delete behaviour in order to make the suite pass. The suite MUST become green because nothing holds the file open, not because removal was attempted repeatedly. In a single-worker, single-process suite a resource still held at teardown is a defect, not environmental flakiness, and absorbing it would satisfy FR-025's acceptance test without meeting its intent.
- **FR-023**: Removing the directory-creation step MUST turn the test suite red. A guarantee whose removal leaves the suite green has no test behind it. **Amended after implementation (2026-09-05).** This standard now applies to one behavioural guarantee — directory creation — rather than two. Removing the loader-scoped configuration was originally required to turn the Linux startup verification red; once the corrupt dependency installation described in FR-003 was repaired, that mutation left every check green on each runtime tested, so the requirement is withdrawn rather than left asserting a result that does not reproduce. The loader-scoped configuration is retained as hardening (FR-003) and is consequently **not** backed by a mutation, which this specification records openly instead of implying coverage it does not have. The cross-platform path guarantees (FR-011, FR-012) remain exempt on their own terms: they are confirmed by review of the changed code together with the full suite running on Linux, as SC-008 records, and no dedicated case-sensitivity test is required.
...
- **SC-006**: The complete pre-existing test suite passes on both Linux and the previously supported environment, with no existing test's expectation modified, and the suite still fails the build if zero tests run. **Status (2026-09-05): partially met.** The Linux half is verified (468 tests, 65 suites, four consecutive runs). The previously-supported-environment half is **not** verified: the suite was never re-run there after this feature's changes, and a Windows-only failure deleting the shared test database has since been reported. Closing that half is tracked in this feature's task list.
...
- **SC-008**: A configured path expressed or resolved on either operating system designates the same location. This is confirmed by the full test suite passing on Linux's case-sensitive filesystem — where every import, migration file read, and configuration path must resolve by exact case — together with review of the changed code confirming it builds paths only through platform-aware facilities. No dedicated case-sensitivity test is required.
...
- **SC-010**: Deleting the directory-creation step turns the suite red. The companion criterion for the loader-scoped configuration was withdrawn after implementation (see FR-003, FR-023): with the dependency installation intact, disabling that configuration leaves every check green, so no mutation stands behind it.
...
- **The Windows cleanup fix is bounded to test teardown (stated 2026-09-05).** It changes test cleanup only. It does not rename the runtime data directory, does not alter production database behaviour, and does not touch the loader-scoped configuration. Those three were settled earlier in this specification and are explicitly out of scope for this correction, so a remedy that reaches for any of them has left its brief.
- **The originating Linux failure was a corrupt dependency installation (discovered during implementation, 2026-09-05).** The installed ORM package was missing 336 of the 444 type-declaration files its own `exports` map declares, so nothing that type-checked against it could resolve — which broke the source loader, the compiled build, and the test runner alike, and presented as a Linux-only fault only because the failure was first met on a freshly installed Linux checkout. Repairing the installation cleared all three. The loader-scoped configuration this specification added is kept as hardening rather than removed, but it is no longer claimed as the remedy for a reproducible defect. The runtime-data-directory half of this specification is unaffected: it was and remains a real defect with tests behind it.
