...
- **FR-023**: Removing the directory-creation step MUST turn the test suite red, and removing the loader-scoped configuration MUST cause the Linux startup verification (FR-021) to fail. A guarantee whose removal leaves the suite green has no test behind it. This standard applies to the two behavioural guarantees this specification introduces — directory creation and the loader-scoped configuration. The cross-platform path guarantees (FR-011, FR-012) are deliberately exempt: they are confirmed by review of the changed code together with the full suite running on Linux, as SC-008 records, and no dedicated case-sensitivity test is required.
...
- **SC-010**: Deleting the directory-creation step turns the suite red, and disabling the loader-scoped configuration turns the Linux startup verification (FR-021) red.
